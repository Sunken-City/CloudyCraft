...
{
    size_t commandNameHash = std::hash<std::string>{}(commandName);
...
CONSOLE_COMMAND(help)
{
    if (args.HasArgs(0))
    {
        ...
        for (auto commandNameFunctionPair : *g_consoleCommands)
        {
            ...
            Console::instance->PrintLine("Currently broken for memory tracking's sake."/*commandNameFunctionPair.first*/, RGBA(red, green, blue));
            i += 1.0f;
        }
        return;
    }
